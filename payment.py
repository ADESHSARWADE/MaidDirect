...
AADHAAR_API_URL = "https://eve.idfy.com/v3/tasks/async/extract/ind_aadhaar"
ACCOUNT_ID = ""
API_KEY = ""
@app.route('/verify-aadhaar', methods=['POST'])
def verify_aadhaar():
  #  aadhaar_number = request.form.get("aadhaar_number")
    aadhaar_file = request.form.get("aadhaar_document")  # URL or BASE64

    # Basic Aadhaar number validation
    # if not aadhaar_number or len(aadhaar_number) != 12 or not aadhaar_number.isdigit():
    #     flash("Invalid Aadhaar number. Please enter a 12-digit valid number.", "error")
    #     return redirect(url_for("aadhaar_verification_page"))

     # Ensure a file is uploaded
    if not aadhaar_file:
        flash("Please upload an Aadhaar document.", "error")
        return redirect(url_for("aadhaar_verification_page"))
    ...
    with open(file_path, "rb") as file:
        encoded_file = file.read().encode("base64")
    ...
    if response.status_code == 200:
        ...
        if response_data.get("status") == "completed":
            ...
            flash("Aadhaar verification failed. Please try again.", "error")
    else:
        flash("Aadhaar verification service is currently unavailable.", "error")

    return redirect(url_for("aadhaar_verification_page"))

@app.route('/aadhaar-verification')
def aadhaar_verification_page():
    ...
    if request.method == 'POST':
        ...
        try:
            msg = Message(
                ...
                sender=email,
                recipients=['']  # Replace with the recipient email
            )
